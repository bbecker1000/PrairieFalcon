...
wd<-setwd("C:/Users/swakamiya/Documents/SFAN/Vital_Signs/Falcons/Analysis/R/GIT/Falcon_MultistateAnalysis")
...
falcons<-as_tibble(falcons)
    ...
    falcons<- falcons %>%
      filter(Year>2002 & Year<2019 & !Survey_Purpose %in% c("Casual", "Unknown") & Territory_Type=="Territory"
    ...
    # Identify surveys conducted where PRFA/PEFA were not detected but a 0 count was not given 
    NoFalcon<- falcons %>%
    ...
    #write_csv(PRFASurveys, path = "PRFASurveys.csv")
    

###### Create datasets for Modeling #######
#- Create PEFA Detections by Year -
    PEFADets<-PEFA %>%
    ...
    SurveyAreaType <- PRFASurveys %>%
      count(Year, Territory_Name, Area_Type)%>%
    ...
    PEFACov<-PRFASurveys%>%
      ...
      spread("Year", "PEFADet", "0")
    colnames(PEFACov)[2:17]<-paste("PEFA",colnames(PEFACov)[2:17], sep="")
    rm(PEFADets)
    
    
#- Create PRFA detection history -
    PRFAStates<- PRFASurveys %>%
      #Sort and group so number visits in the correct order
      arrange(Territory_Name, Start_Date)%>% 
      group_by(Territory_Name, Year)%>%
      mutate(Visit=row_number())%>%
      #Create YearVisit field
    ...
    PRFADetectHistory<-PRFAStates %>%
      ...
      #write_csv(PRFADetectHistory, path = "PRFADetectHistory.csv")
    
            #---check lists---
            # #Create PRFA annual survey list to determine if years that territories were not surveyed match those of the PEFA covariate list
            # PRFACheck <-PRFADetectHistory %>%
            #   gather(Session, Detection, 3:276) %>%
